...
class Solution{
    ...
    {
        HashMap<Integer,Integer>sia=new HashMap<>();
        ArrayList<Integer>kia=new ArrayList<>();
        for(int i=0;i<v1.size();i++)
        {
         sia.put(v1.get(i),sia.getOrDefault((v1.get(i)),0)+1);
        }
        for(int j=0;j<v2.size();j++)
        {
          if(sia.containsKey(v2.get(j))&& sia.get(v2.get(j))>0)
          {
            kia.add(v2.get(j));
            sia.put(v2.get(j),sia.get(v2.get(j))-1);
          }
        }
